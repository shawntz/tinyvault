...
def wrap_key():
    ...
    if request.method == 'OPTIONS':
        ...
        sanitized_origin = re.sub(r'[^\x20-\x7E]', '', request.headers.get('Origin', '')).replace('\r', '').replace('\n', '')
        ...
        if origin:
            ...
            if parsed.hostname is None:
                ...
                logger.warning("Malformed or missing hostname in Origin header: '%s'", safe_origin)
            else:
    ...
    sanitized_headers_str = '; '.join(
        f"{re.sub(r'[^\x20-\x7E]', '', str(k)).replace('\r', '').replace('\n', '')}: {re.sub(r'[^\x20-\x7E]', '', str(v)).replace('\r', '').replace('\n', '')}"
        for k, v in request.headers.items()
    )
    logger.info("Headers: %s", sanitized_headers_str)
    ...
    try:
        ...
        if data:
            ...
            if isinstance(authorization, dict):
                ...
                # Sanitize any representation of authorization to prevent log injection and log flooding
                safe_auth = repr(authorization).replace('\r', '').replace('\n', '')[:200]
            logger.info(f"Full request body (excluding sensitive key): {{'authentication': '***', 'authorization': {safe_auth}, 'key': '[REDACTED]'}}")
